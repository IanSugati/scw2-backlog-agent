...
def bullets(rows, limit: int):
    ...
    for row in rows[:limit]:
        # row: (key, summary, status, project_key, extra)
...
def main():
    ...
    assigned_meta = {}  # key -> {summary,status,project,duedate}
    ...
    def ensure_worked_meta(issue_obj):
        key = issue_obj["key"]
    ...
    # -------------------------------------------------
    # D) Other open tasks assigned to Naval not due this week (or no due date)
    # -------------------------------------------------
